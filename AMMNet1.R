...
getwd()
mdata<-read.csv("Malaria-Data.csv")
View(mdata)
...
sum(is.na(mdata))###Check for missing values
...
plot(factor(severe_maleria), names= c('Not severe', 'severe'), col=c(2,3), ylim=c(0, 600), ylab='Respondent', xlab='BMI Category')
box()
##Class Imbalance
...
set.seed(123)
tic()
SvmModel <- train(factor(severe_maleria)~., data=train, method="svmRadial", preProc=c("center", "scale"), trControl=control, na.action = na.omit)
toc()
SvmModel
...
set.seed(123)
tic()
RFModel <- train(factor(severe_maleria)~., data=train, method="rf", preProc=c("center", "scale"), trControl=control)
toc()
RFModel
...
set.seed(123)
lrModel <- train(factor(severe_maleria)~., data=train, method="glm", preProc=c("center", "scale"),trControl=control)
lrModel
...
set.seed(123)
nnModel <- train(factor(severe_maleria)~., data=train, method="nnet", preProc=c("center", "scale"),trControl=control)
nnModel
...
set.seed(123)
nbModel <- train(factor(severe_maleria)~., data=train, method="nb",preProc=c("center", "scale"), trControl=control)
nbModel
...
set.seed(123)
ldaModel <- train(factor(severe_maleria)~., data=train, method="lda", preProc=c("center", "scale"),trControl=control)
ldaModel
...
set.seed(123)
lvqModel <- train(severe_maleria~., data=train, method="lvq", preProc=c("center", "scale"),trControl=control)
lvqModel
...
set.seed(123)
bagModel <- train(factor(severe_maleria)~., data=train, method="treebag", preProc=c("center", "scale"),trControl=control)
bagModel
...
## Train Neural Net model
set.seed(123)
over.nnModel <- train(factor(severe_maleria)~., data=over, method="nnet", trControl=control)
over.nnModel
over.nnpred=predict(over.nnModel,newdata = test)
over.nn.cM <- confusionMatrix(over.nnpred,as.factor(test$severe_maleria), positive = 'severe', mode='everything')
over.nn.cM
#plot confusion matrix
over.nn.cM$table
...
## Train Naive Bayes model
set.seed(123)
over.nbModel <- train(factor(severe_maleria)~., data=over, method="nb", trControl=control)
over.nbModel
over.nbpred=predict(over.nbModel,newdata = test)
over.nb.cM <- confusionMatrix(over.nbpred,as.factor(test$severe_maleria), positive = 'severe', mode='everything')
over.nb.cM
#plot confusion matrix
over.nb.cM$table
...
## Train Linear Discriminant Analysis model
set.seed(123)
over.ldaModel <- train(factor(severe_maleria)~., data=over, method="lda", trControl=control)
over.ldaModel
over.ldapred=predict(ldaModel,newdata = test)
over.lda.cM <- confusionMatrix(ldapred,as.factor(test$severe_maleria), positive = 'severe', mode='everything')
over.lda.cM
##plot confusion matrix
over.lda.cM$table
...
## Train Linear Vector Quantization Model
set.seed(123)
over.lvqModel <- train(factor(severe_maleria)~., data=over, method="lvq", trControl=control)
over.lvqModel
over.lvqpred=predict(lvqModel,newdata = test)
over.lvq.cM <- confusionMatrix(lvqpred,as.factor(test$severe_maleria), positive = 'severe', mode='everything')
over.lvq.cM
##plot confusion matrix
over.lvq.cM$table
...
## Train Bagging model
set.seed(123)
over.bagModel <- train(factor(severe_maleria)~., data=over, method="treebag", trControl=control)
over.bagModel
over.bagpred=predict(over.bagModel,newdata = test)
over.bag.cM <- confusionMatrix(over.bagpred,as.factor(test$severe_maleria), positive = 'severe', mode='everything')
over.bag.cM
#plot confusion matrix
over.bag.cM$table
...
## Train a Boosting model
set.seed(123)
over.boModel <- train(factor(severe_maleria)~., data=over, method="ada", trControl=control)
over.boModel
over.bopred=predict(over.boModel,newdata = test)
over.bo.cM <- confusionMatrix(over.bopred,as.factor(test$severe_maleria), positive = 'severe', mode='everything')
over.bo.cM 
#plot confusion matrix
over.bo.cM$table
